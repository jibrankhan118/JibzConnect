...
// Create a channel — creator automatically becomes its admin
...
// List members of a channel (with role)
...
router.post("/:id/members", requireChannelAdmin, async (req, res) => {
  try {
...
router.delete("/:id/members/:userId", requireChannelAdmin, async (req, res) => {
  try {
    const { id: channelId, userId } = req.params;

    const membership = await ChannelMember.findOne({ where: { channelId, userId } });
    if (!membership) {
      return res.status(404).json({ message: "This user is not a member of the channel." });
    }
...
router.patch("/:id/members/:userId/role", requireChannelAdmin, async (req, res) => {
  try {
    const { id: channelId, userId } = req.params;
    const { role } = req.body;
